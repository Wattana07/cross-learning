...
import { createRoomCategory } from './api'
import { DoorOpen, FileText } from 'lucide-react'

interface CreateRoomCategoryModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}

export function CreateRoomCategoryModal({ isOpen, onClose, onSuccess }: CreateRoomCategoryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'เกิดข้อผิดพลาดในการสร้างหมวดหมู่ห้องประชุม')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="เพิ่มหมวดหมู่ห้องประชุมใหม่"
      description="สร้างหมวดหมู่ห้องประชุมใหม่"
      size="md"
    >
