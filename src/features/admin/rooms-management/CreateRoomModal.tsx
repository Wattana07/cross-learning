...
export function CreateRoomModal({ isOpen, onClose, onSuccess, categories, roomTypes }: CreateRoomModalProps) {
    ...
    if (formData.room_category_id) {
      fetchTableLayoutsByCategory(formData.room_category_id)
        .then((layouts) => {
          ...
          // Reset table_layout_id if current selection is not in new list
          if (formData.table_layout_id && !layouts.find(l => l.id === formData.table_layout_id)) {
  ...
  return (
    <Modal
      ...
      description="สร้างห้องประชุมใหม่"
      size="md"
    >
