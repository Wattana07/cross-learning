...
import { createSubject, updateSubject, fetchCategoriesForSelect } from './api'
import { uploadSubjectCover } from '@/lib/storage'
import { BookMarked, FileText, Eye, EyeOff, Camera, X, FolderOpen } from 'lucide-react'
import { cn } from '@/lib/utils'
import type { ContentStatus, UnlockMode } from '@/lib/database.types'
...
export function CreateSubjectModal({ isOpen, onClose, onSuccess }: CreateSubjectModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    level: 'beginner',
    unlock_mode: 'sequential' as UnlockMode,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (coverFile) {
        try {
          const coverPath = await uploadSubjectCover(coverFile, subject.id)
      ...
      setError(err.message || 'เกิดข้อผิดพลาดในการสร้างวิชา')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Modal
      ...
      onClose={handleClose}
      title="เพิ่มวิชาใหม่"
      description="สร้างวิชาการเรียนรู้ใหม่"
      size="md"
    >
