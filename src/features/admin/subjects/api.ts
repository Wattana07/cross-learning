...
export async function createSubject(subject: {
  ...
  if (error) {
    await logger.error('subject_update', {
  ...
  await logger.success('subject_update', {
    resourceType: 'subject',
    resourceId: id,
    details: updates,
  })
...
export async function updateSubject(
  ...
  if (error) {
    await logger.error('subject_create', {
      resourceType: 'subject',
      errorMessage: error.message,
      details: { title: subject.title, category_id: subject.category_id },
    })
    throw error
  }
  
  await logger.success('subject_create', {
    resourceType: 'subject',
    resourceId: data.id,
    details: { title: data.title, category_id: data.category_id, status: data.status },
  })
...
export async function deleteSubject(id: string): Promise<void> {
  ...
  if (error) {
    await logger.error('subject_delete', {
      resourceType: 'subject',
      resourceId: id,
      errorMessage: error.message,
    })
  ...
  await logger.success('subject_delete', {
    resourceType: 'subject',
    resourceId: id,
  })
}
