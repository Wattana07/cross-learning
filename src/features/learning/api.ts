...
export async function saveEpisodeProgress(
  ...
  const wasCompleted = existing?.completed_at !== null || (existing?.watched_percent ?? 0) >= 90
  ...
  const isNowCompleted = progressData.completed_at !== null || watchedPercent >= 90
  ...
  if (existing) {
    ...
    if (error) {
      await logger.error('episode_watch', {
    ...
    // Log watch progress
    await logger.info('episode_watch', {
      resourceType: 'episode',
      resourceId: episodeId,
      details: { watched_percent: watchedPercent, last_position: lastPosition },
    })
    
    // Log completion if just completed
    if (isNowCompleted && !wasCompleted) {
      await logger.success('episode_complete', {
        resourceType: 'episode',
        resourceId: episodeId,
        details: { watched_percent: watchedPercent },
      })
    }
